...
def uc_frontend(filename, write_types, write_graph, frontend_phase):
    ...
    check_errors(ucerror.error_count(), 0)
...
def main():
    ...
    group.add_argument('-C', '--code-gen-only', action='store_true',
                       help='generate code only for the input uC file')
    group.add_argument('-A', '--analyze-and-code-gen',
                       action='store_true',
                       help='perform static analysis and generate code '
                       'for the input uC file')
    group.add_argument('-P', '--parse-only',
                       action='store_true',
                       help='parse the input uC file only')
    ...
    if args.parse_only:
        ...
        args.frontend_phase = 0
    elif not args.frontend_phase and not args.code_gen_only:
