...
def check_names(tree, global_env):
    ...
    they are unique in the scope of the type or function. Resolves the
    types of name expressions.
    """
    ctx = uccontext.PhaseContext(4, global_env)
    # used to look up local names
    ctx['local_env'] = None
    # used to check whether a variable is used in its own initialization
    ctx['initializing'] = None
